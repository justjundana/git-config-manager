...
function Remove-GitCredential {
    ...
    # Common hosts
    # NOTE: $host is a PowerShell automatic variable (the host UI object) and
    # must not be used as a loop variable — assigning to it aborts the function.
    foreach ($providerHost in @("https://github.com", "https://gitlab.com", "https://bitbucket.org", "https://dev.azure.com")) {
        $helper = & git config --global "credential.$providerHost.helper" 2>$null
        if ($helper) {
            & git config --global --unset-all "credential.$providerHost.helper" 2>$null
            Print-Success "Removed credential helper for $providerHost"
            $cleaned = $true
        }
        $user = & git config --global "credential.$providerHost.username" 2>$null
        if ($user) {
            & git config --global --unset-all "credential.$providerHost.username" 2>$null
            Print-Success "Removed credential username for $providerHost"
            $cleaned = $true
        }
    }
...
# Guessing "id_<type>_<profile>" from profile names both deletes keys the user
# made themselves that happen to match the pattern, and misses GCM keys stored
# anywhere else. Returns $null when the ledger exists but cannot be parsed, so
# callers abort instead of falling back to guesswork.
function Get-GcmLedger {
    $ledgerPath = Join-Path (Join-Path $env:USERPROFILE ".gcm") "generated-keys.json"
    if (-not (Test-Path $ledgerPath)) {
        return [PSCustomObject]@{ Ssh = @(); Gpg = @() }
    }
    try {
        $raw = Get-Content -Path $ledgerPath -Raw -ErrorAction Stop
    ...
    $sshPaths = @()
    foreach ($entry in @($data.ssh)) {
        if ($entry -and $entry.key_path -and -not [string]::IsNullOrWhiteSpace($entry.key_path)) {
            $sshPaths += $entry.key_path.Trim()
        }
    }
    $gpgIds = @()
    foreach ($entry in @($data.gpg)) {
        if ($entry -and $entry.key_id -and -not [string]::IsNullOrWhiteSpace($entry.key_id)) {
            $gpgIds += $entry.key_id.Trim()
        }
    }

    return [PSCustomObject]@{ Ssh = $sshPaths; Gpg = $gpgIds }
}
...
function Remove-SshKeys {
    ...
    $ledger = Get-GcmLedger
    if ($null -eq $ledger) {
        Print-Warning "Cannot read the generated-keys ledger - no SSH key will be removed"
        Print-Info "  Remove any GCM keys by hand after checking ~\.gcm\generated-keys.json"
        return
    }

    $sshFound = @()
    foreach ($keyPath in $ledger.Ssh) {
...
function Remove-GpgKeys {
    ...
    # Scraping key_id from profile YAML cannot tell a key GCM generated from one
    # the user generated and merely attached to a profile - and deleting a
    # secret key is irreversible.
    $ledger = Get-GcmLedger
    if ($null -eq $ledger) {
        Print-Warning "Cannot read the generated-keys ledger - no GPG key will be removed"
        Print-Info "  Remove any GCM keys by hand after checking ~\.gcm\generated-keys.json"
        return
    }
    $gpgKeyIds = $ledger.Gpg

    if ($gpgKeyIds.Count -eq 0) {
        Print-Info "No GCM-generated GPG keys recorded in the ledger"
        return
    }
