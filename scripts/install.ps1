...
# This script installs gcm to $env:USERPROFILE\.local\bin and only mutates PATH/shell config when explicitly requested.
...
function Get-LatestVersion {
    ...
    try {
        $response = Invoke-RestMethod -Uri "https://api.github.com/repos/justjundana/git-config-manager/releases/latest" -TimeoutSec 30
...
# Download, checksum-verify, and install the release binary.
function Download-Binary {
    ...
    $baseUrl = "https://github.com/justjundana/git-config-manager/releases/download/$DownloadVersion"
    ...
    Print-Info "Binary URL: $binaryUrl"
    ...
    if (-not $Quiet) {
        Write-Host "   $($Colors.Dim)Downloading release binary...$($Colors.Reset)"
    ...
    try {
        Save-Url $binaryUrl $downloadPath
        ...
        Print-Error "Failed to download gcm release binary or checksums"
        Print-Info "Error: $($_.Exception.Message)"
    ...
    if (-not $Quiet) {
        Write-Host "   $($Colors.Green)$($Icons.Checkmark)$($Colors.Reset) Downloaded release binary and checksums."
    }

    if (-not (Test-ReleaseChecksum $downloadPath $checksumsPath)) {
        Remove-Item $tempDir -Recurse -Force -ErrorAction SilentlyContinue
        exit 1
    }

    Copy-Item -LiteralPath $downloadPath -Destination $binaryPath -Force
    Remove-Item $tempDir -Recurse -Force -ErrorAction SilentlyContinue
