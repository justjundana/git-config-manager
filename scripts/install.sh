...
# This script installs gcm to $HOME/.local/bin and only mutates shell config when explicitly requested.
set -e
...
get_latest_version() {
    ...
    local version=""
    if command -v curl >/dev/null 2>&1; then
        version=$(curl -fsSL https://api.github.com/repos/justjundana/git-config-manager/releases/latest | grep '"tag_name"' | sed -E 's/.*"([^"]+)".*/\1/')
    elif command -v wget >/dev/null 2>&1; then
        version=$(wget -qO- https://api.github.com/repos/justjundana/git-config-manager/releases/latest | grep '"tag_name"' | sed -E 's/.*"([^"]+)".*/\1/')
    else
        print_error "Either curl or wget is required to download gcm"
        exit 1
    fi
...
# Verify binary after checksum-protected download.
verify_binary() {
    local binary_path="$1"
...
download_binary() {
    ...
    local base_url="https://github.com/justjundana/git-config-manager/releases/download/${version}"
    local binary_url="${base_url}/${asset_name}"
    local checksums_url="${base_url}/checksums.txt"
    local tmp_dir binary_path checksums_path
    ...
    echo -e "   ${DIM}Downloading release binary...${NC}"
    if ! download_file "$binary_url" "$binary_path"; then
        print_error "Failed to download gcm binary from $binary_url"
        rm -rf "$tmp_dir"
        exit 1
    fi
    ...
    echo -e "   ${GREEN}${CHECKMARK}${NC} Downloaded release binary and checksums."
