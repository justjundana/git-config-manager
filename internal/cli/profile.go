...
func newProfileDeleteCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(_ *cobra.Command, args []string) error {
			...
			//
			// Only key material GCM generated may be deleted: a profile can
			// just as well point at a key the user already had, and the ledger
			// is the source of truth for that distinction. On a ledger read
			// error we keep the key — losing a user's private key is far worse
			// than leaving a generated one behind.
			if p.SSH != nil && p.SSH.KeyPath != "" {
				privKey := p.SSH.KeyPath
				pubKey := privKey + ".pub"
				generated, ledgerErr := ctr.KeyLedger.HasSSH(privKey)
				if ledgerErr != nil {
					_ui.Warning("Could not read the generated-keys ledger: %v", ledgerErr)
				}
				removedAny := false
				if generated && ledgerErr == nil {
					...
					_ = ctr.KeyLedger.RemoveSSH(privKey)
				} else {
					_ui.Info("Keeping SSH key %s (not generated by GCM)", privKey)
				}
			...
			// Clean up associated GPG key. Same rule as the SSH key above: a
			// secret key GCM did not generate is never removed from the
			// keyring, because deleting one is irreversible.
			if p.GPG != nil && p.GPG.KeyID != "" {
				gpgLocalDeleted := false
				generatedGPG, gpgLedgerErr := ctr.KeyLedger.HasGPG(p.GPG.KeyID)
				if gpgLedgerErr != nil {
					_ui.Warning("Could not read the generated-keys ledger: %v", gpgLedgerErr)
				}
				if !generatedGPG || gpgLedgerErr != nil {
					_ui.Info("Keeping GPG key %s (not generated by GCM)", p.GPG.KeyID)
				} else if ctr.GPGManager.IsInstalled() {
					if err := ctr.GPGManager.Delete(p.GPG.KeyID); err != nil {
						...
						_ = ctr.KeyLedger.RemoveGPG(p.GPG.KeyID)
					}
				}
